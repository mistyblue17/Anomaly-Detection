...
newruns <- read_csv("results_summary_bycrop_testnewrun1.csv")
...
# reference: https://www.rdocumentation.org/packages/dbscan/versions/1.1-3/topics/dbscan
### CASSAVA ###
...
### subset new run by crop
# diffyield <- function(crop) {
#    switch("crop",  
#          "cassava" = newdata$`yield (kg/ha)`- cassava_0_yield,
#          "groundnuts" = newdata$`yield (kg/ha)`- groundnuts_0_yield,
#          "maize" = newdata$`yield (kg/ha)`- maize_0_yield,
#          "sesame" = newdata$`yield (kg/ha)`- sesame_0_yield,
#          "sorghum" = newdata$`yield (kg/ha)`- sorghum_0_yield)
# }
#   
# newdata$diffyield <- diffyield(newdata$crop)

# newdata$diffyield <- newdata %>% 
#   if (crop == "cassava") {
#     newdata$`yield (kg/ha)`- cassava_0_yield
#   } else if (crop == "groundnuts") {
#     newdata$`yield (kg/ha)`- groundnuts_0_yield
#   } else if (crop == "maize") {
#     newdata$`yield (kg/ha)`- maize_0_yield
#   } else if (crop == "sesame") {
#     newdata$`yield (kg/ha)`- sesame_0_yield
#   } else if (crop == "sorghum") {
#     newdata$`yield (kg/ha)`- sorghum_0_yield
#   }


# identify crop in run that was given fertilizer subsidy
newdata_select <- newdata %>% 
  filter(newdata$`fert_subsidy (%)` != 0)

# use that crop's baseline values to calculate difference 
# crop_0_yield <- paste(newdata_select$crop, "_0_yield", sep = "")
# crop_0_fert <- paste(newdata_select$crop, "_0_fert", sep = "") 
newdata$diff_yield <- newdata$`yield (kg/ha)` - maize_0_yield
newdata$diff_fert <- newdata$`Nfert (kg/ha)`- maize_0_fert

# select columns to be compared to DBSCAN cluster results
newdata_select <- newdata[ , c("diff_yield", "diff_fert")]

# trim allcrops_subruns_select so that columns match those analyzed by DBSCAN
...
predict(res_dbscan, newdata_select, data = maize_subruns_select)
predict(res_dbscan, newdata_select, data = groundnuts_subruns_select)
...
# normalize using differnce from baseline for each crop
